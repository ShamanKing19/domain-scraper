...
class MixedParser:
    ...
    async def __save_site_info(self, id, domain, zone, real_url, html):
        ...
        bitrix24url = 'https://46.8.141.57/rest/1/c5gdf869ny8ekfpq/crm.deal.add.json'
        HeadersBitrix = {"Content-Type": "application/json"}
        fields = {
        "fields": {
                "TITLE": {title},
                "CURRENCY_ID": "RUB",
                "OPPORTUNITY": random.randint(1,100000),
                "UF_CRM_1655194508": {tag_id},
                "UF_CRM_1655194854": {city},
                "UF_CRM_1655195856": {emails},
                "UF_CRM_1655195869": {numbers},
                "UF_CRM_1655194912": {inns},
                "UF_CRM_1655194921": {cms}
            }
        }


        response = requests.post(bitrix24url, data=json.dumps(fields), headers=HeadersBitrix, verify=False)
    ...
    async def __make_domain_request(self, domain_base_info):
        ...
        session = aiohttp.ClientSession(connector=aiohttp.TCPConnector(), timeout=session_timeout)
...
def main():
    ...
    portion = 100 # Это одновременно обрабатываемая порция
    start_time = time.time() 
