...
class MixedParser:
    ...
    async def __save_site_info(self, id, domain, zone, response, is_ssl, is_https_redirect, html):
        ...
        self.db.make_db_request(f"""
            INSERT INTO {self.domain_info_table_name} (domain_id, title, description, city, inn, cms, is_www, is_ssl, is_https_redirect, ip, tag_id) 
            VALUE ({id}, '{title}', '{description}', '{city}', '{inn}', '{cms}', '{www}', '{is_ssl}', '{is_https_redirect}', '{ip}', {tag_id})
            ON DUPLICATE KEY UPDATE title='{title}', description='{description}', city='{city}', inn='{inn}', cms='{cms}', is_www='{www}', is_ssl='{is_ssl}', is_https_redirect='{is_https_redirect}',  ip='{ip}', tag_id={tag_id}
        """)
